...
<main class="main cart">
    ...
    <!-- Start Main -->
    <?php
    $paytr_settings = $dbh->query("SELECT * FROM paytr_settings WHERE id = 1 ", PDO::FETCH_ASSOC);
    ...
    $siparis = $dbh -> prepare("SELECT * FROM product_order WHERE sipno = ?");
    $siparis-> execute(array($_GET["sip"]));
    ...
    $payment_amount = $siparis->total_amount*100; //9.99 için 9.99 * 100 = 999 gönderilmelidir.
    #
    ## Sipariş numarası: Her işlemde benzersiz olmalıdır!! Bu bilgi bildirim sayfanıza yapılacak bildirimde geri gönderilir.
    $merchant_oid = $sipno;
    #
    ## Müşterinizin sitenizde kayıtlı veya form aracılığıyla aldığınız ad ve soyad bilgisi
    $user_name =  $siparis->name_surname;
    #
    ## Müşterinizin sitenizde kayıtlı veya form aracılığıyla aldığınız adres bilgisi
    $user_address = "istanbul";
    #
    ## Müşterinizin sitenizde kayıtlı veya form aracılığıyla aldığınız telefon bilgisi
    $user_phone = $siparis->phone;
    #
    ## Başarılı ödeme sonrası müşterinizin yönlendirileceği sayfa
    ## !!! Bu sayfa siparişi onaylayacağınız sayfa değildir! Yalnızca müşterinizi bilgilendireceğiniz sayfadır!
    ## !!! Siparişi onaylayacağız sayfa "Bildirim URL" sayfasıdır (Bakınız: 2.ADIM Klasörü).
    $merchant_ok_url =  $paytr_settings["web_url"]."siparis-alindi.php";
    #
